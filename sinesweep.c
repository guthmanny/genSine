                          ...
                          void *userData )
{
    ...
    for( i=0; i<framesPerBuffer; i++ )
    {
        ...
        *out++ = gain * sin(data->right_phase);  /* right */
        data->left_phase += TWO_PI * (data->freq_sweep / SAMPLE_RATE);
        if( data->left_phase >= TWO_PI ) data->left_phase -= TWO_PI;
        data->right_phase += TWO_PI * (data->freq_sweep / SAMPLE_RATE);
        if( data->right_phase >= TWO_PI ) data->right_phase -= TWO_PI;
        data->freq_sweep += data->inc_freq;
        if(data->freq_sweep >=20000.0) data->freq_sweep = start_freq;
    }
...
int main(void)
{
    ...
    data.freq_sweep = start_freq;
    data.inc_freq = (end_freq - start_freq)/(NUM_SECONDS * SAMPLE_RATE);
